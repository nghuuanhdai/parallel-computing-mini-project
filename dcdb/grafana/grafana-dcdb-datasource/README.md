## DCDB Datasource plugin - a generic backend datasource for DCDB

This plugin is based on the simple JSON datasource plugin for Grafana. The backend server implements the following URLs

Your backend needs to implement 4 urls:

 * `/` returns 200 ok. Used for "Test connection" on the datasource definition config page.
 * `/search` used by the find metric options on the query tab in panels.
 * `/query` should return metrics based on input.
 * `/annotations` should return annotations (not yet implemented)
 * `/tag-keys` should return tag keys for ad hoc filters (not yet implemented).
 * `/tag-values` should return tag values for ad hoc filters (not yet implemented).
